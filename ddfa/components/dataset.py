...
class ImageNet50(Dataset):
    def get_data(self, data_root, batch_size, dataset_seed):
        ...
        split_seed    = np.random.randint(1, 4_000_000_000)
        train_seed    = np.random.randint(1, 4_000_000_000)
        valid_seed    = np.random.randint(1, 4_000_000_000)
        test_seed     = np.random.randint(1, 4_000_000_000)
        ...
        train_data, _ = torch.utils.data.random_split(
            train_base, [n_train, n_valid], generator=torch.Generator().manual_seed(split_seed)
        )
        train_base = ImageNetSubset(subset_file='data_utils/imagenet_50.txt', root=data_root, split='train', transform=transforms.Compose([
                                        transforms.Resize(256),
        ...
        train_data = torch.utils.data.Subset(train_data, torch.randperm(len(train_data),generator=torch.Generator().manual_seed(train_seed)).tolist())
        valid_data = torch.utils.data.Subset(valid_data, torch.randperm(len(valid_data),generator=torch.Generator().manual_seed(valid_seed)).tolist())
        test_data  = torch.utils.data.Subset(test_data,  torch.randperm(len(test_data),generator=torch.Generator().manual_seed(test_seed)).tolist())
